...
from utils import shark_icon, pilot_icon, images, get_images, update_images
...
shark0 = make_shark()
pilot0 = make_pilotfish(shark0)

shark_list = [shark0]
pilotfish_list = [pilot0]
    ...
    for shark in shark_list:
        ...
        if event == sg.TIMEOUT_KEY:
            ...
            if (shark_x, shark_y) != (last_shark_x, last_shark_y):
                ...
                position_pilotfish()
                last_shark_x, last_shark_y = shark_x, shark_y
            # TODO: probably gonna need to make (shark_x, shark_y, last_direction,
            #       last_shark_x, last_shark_y) into window meta data
            continue
        elif event == 'Add another pair':
            next = len(shark_list)
            eval('shark + str(next) = make_shark()')
            eval('pilot + str(next)) = make_pilotfish(eval(shark + str(next))')
            shark_list.append(eval(shark + str(next)))
            pilotfish_list.append(eval(pilot + str(next)))
